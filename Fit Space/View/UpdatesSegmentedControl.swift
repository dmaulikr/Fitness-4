...
public enum CursorPositionForUpdates
{
    case Top
    case Bottom
}

public class UpdatesSegmentedControl : UIControl {
    ...
    public var cursorPosition:CursorPositionForUpdates = CursorPositionForUpdates.Bottom
        {
        didSet{
            if views.count > 0
            {
    ...
    public var selectedColor:UIColor? = UIColor.clearColor()
        {
        didSet{
    ...
    public var unselectedColor:UIColor? = UIColor.clearColor()
        {
        didSet{
    ...
    {
        tapGestureRecogniser = UITapGestureRecognizer(target: self, action: #selector(UpdatesSegmentedControl.didTap(_:)))
    ...
    public func setItems(items:[String])
    {
        ...
        for i in 0..<items.count
        {
            ...
            if(cursorPosition == CursorPositionForUpdates.Top)
            {
            ...
            UIView.animateWithDuration(duration, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 10, options: [UIViewAnimationOptions.CurveEaseInOut, UIViewAnimationOptions.AllowUserInteraction], animations:
                {
                    ...
                    if let cur = self.cursor
                    {
                        cur.center.x = newView.center.x - 2
                    }
                },
